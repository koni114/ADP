...
# 
# 02. 기초 통계량
# - summary : 다소 복잡한 방법을 이용하여 1분위수와 3분위수 계산  
# - fivenum  : 중앙 값을 기준으로 좌측 데이터의 중앙이 1분위, 우측 데이터의 중앙이 3분위
# 
# 03. 표본 추출
# - 단순 임의 추출 : sample
# - 층화 추출        : strata
...
# 
# 04. 분할표
# - 분할표 작성 : xtabs, margin.table
# - 독립성 검정 : 행, 열에 나열된 변수가 독립인지 검정
# - 카이제곱검정
# - 피셔의 정확검정
# - 맥니마 검정
# 
...
# 연속형 데이터의 확률을 표현
# f(x) 또는 p(x)를 계산
# 함수 앞에 d가 있으면, 확률밀도함수 또는 확률질량함수
dnorm(x = 3, mean = 1, sd = 1)

# 1.3 분포함수
# 확률 변수 X가 x 이하의 값을 가질 확률 값을 return
# F(x) = P(X <= x) 
# 함수 앞에 p가 있으면, 분포함수(Distribution function)
...
library(sampling)
x <- sampling::strata(  stratanames = c("Species")
                      , size        = c(3, 3, 3)
                      , method      = 'srswor'
                      , data        = iris)
...
# 결과 해석
# p-value 값이 0.05보다 크므로, 귀무 가설을 채택하여 성별과 운동은 독립이다
chisq.test(xt) 
